...
if [[ ${#TASK_IDS[@]} -gt 0 ]]; then
	for ROBOT_ID in "${ROBOT_IDS[@]}"; do
		...
		for TASK_ID in "${TASK_IDS[@]}"; do
			...
			EXPRESSION="m.header.frame_id.endswith('${ROBOT_ID}') and m.task_id == '${TASK_ID}'"
...
for ((i = 0; i < ${#ROBOT_IDS[@]}; ++i)); do
	...
	if [[ ! -f ${YAML_FILE} ]]; then
		echo "ERROR: Enter the yaml of the desired robot or the id of the desired task, please."
		echo "Aborting..."
		exit 
	fi
	...
	for ((j = 0; j < ${NUMBER_OF_TASKS}; ++j)); do
		...
		OUTPUT_BAG_FILE=../bag/${ROBOT_ID}-${TASK_ID}-motivation.bag
		OUTPUT_CSV_FILE=../bag/${ROBOT_ID}-${TASK_ID}-motivation.csv
		OUTPUT_CSV_FILE_HEADER="%time,impatience,acquiescent,suppressed,resetted,aplicable,motivation (for the ${ROBOT_ID}/${TASK_ID} behaviour set)";
		EXPRESSION="m.header.frame_id.endswith('${ROBOT_ID}') and m.task_id == '${TASK_ID}'"
		PATTERN="2d;s/.*,.*,\(.*\),.*${ROBOT_ID},${TASK_ID}/\1/g;"
