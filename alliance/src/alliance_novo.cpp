...
void report()
{
  ...
  goal_d = sqrt((x_goal - current_position.position.x) *
                    (x_goal - current_position.position.x) +
                (y_goal - current_position.position.y) *
                    (y_goal - current_position.position.y));
  beta = atan2((y_goal - current_position.position.y) *
                   (y_goal - current_position.position.y),
               (x_goal - current_position.position.x) *
                   (x_goal - current_position.position.x));
  alpha = -th + beta;
...
void boundary_overwatch()
{
  ...
  if (current_position.position.x < 9.0 && current_position.position.x > -7.0 &&
      current_position.position.y < 6.0 && current_position.position.y > -6.0)
  {
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher position_pub =
      n.advertise<geometry_msgs::Pose>("Robot0_Position", 5);
  ...
  while (ros::ok() and task_done[3] == 0)
  {
    ...
    {
      boundary_overwatch();
      if (current_position.position.x > 0.0 &&
          current_position.position.y > 6.0 && pos_count < 1)
      {
        pos_wall_initx = current_position.position.x;
        pos_wall_inity = current_position.position.y;
        d_init = d;
        pos_count++;
      ...
      {
        cur_x = current_position.position.x;
        cur_y = current_position.position.y;
    ...
    current_position.robot_id = id;
    current_position.position.x = current_posx + xi;
    current_position.position.y = current_posy + yi;
    ...
    ROS_INFO(
        ...
        current_position.position.x, current_position.position.y, d);
